...
def group(values: tp.List[T], n: int) -> tp.List[tp.List[T]]:
    ...
    """
    if len(values) % n != 0:
        raise ValueError(f"Невозможно составить матрицу, так как длина списка {len(values)} не кратна значению n {n}")
...
def get_col(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    """
    col_indx = pos[1]
    return [row[col_indx] for row in grid]
...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    row_start = (pos[0] // 3) * 3
    col_start = (pos[1] // 3) * 3
    return [grid[i][j] for i in range(row_start, row_start + 3) for j in range(col_start, col_start + 3)]
